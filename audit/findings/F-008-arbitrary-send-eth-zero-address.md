...
### Deployed Addresses Verified

This finding was verified against the deployed bytecode at the following live addresses (canonical source: [`src/DeployedAddresses.sol`](src/DeployedAddresses.sol)):

| Contract                  | Mainnet                                      | Holesky (Testnet)                            |
| ------------------------- | -------------------------------------------- | -------------------------------------------- |
| CL Fee Dispatcher (impl)  | `0x462Dd07A79e5DDfBe0C171449C5c01788d5d03C3` | `0xD36B422a7EE65219732724d849B8b6BceD6155Fe` |
| CL Fee Dispatcher (proxy) | `0xE8EC6F702D68ded71112031D78bBFf959c7234C7` | `0x50Dba42662FD69f5Fd9236540aaD9f99f7F6b3b2` |
| EL Fee Dispatcher (impl)  | `0xca4DD914fA713214844c84F153A5e1627536a7fC` | `0xa69dDEBd0B6893A6F3d34A5df610d0E2ED433D18` |
| EL Fee Dispatcher (proxy) | `0x72b4C52f18f52EbA3E4290a002dF7c387427b058` | `0x639d818639B85a1892Bfbb40Bd724b4Ddea43C0C` |

Both dispatcher implementations share the same source code on Mainnet and Holesky. The source code verification confirms the vulnerable `dispatch()` functions are identical across all deployments.
...
The bounty scope states: _"The Operator, Admin, and Proxy Admin are trusted to behave properly and in the best interest of users. They should not be considered as malicious."_
...
4. **Default `address(0)` return values for unconfigured validators**: The staking contract at [`StakingContract.sol:259-260`](src/contracts/StakingContract.sol:259) states: _"In case the validator is not enabled, it will return address(0)"_. A validator removed from the contract causes `getWithdrawerFromPublicKeyRoot()` to return `address(0)` — the dispatchers do not validate this.
...
The scope excludes _"Issues that are ultimately user errors (e.g., transfers to `address(0)`)"_. This finding does not fall under this exclusion for five reasons:
...
The scope excludes _"Design choices related to protocol"_. The inconsistent `_checkAddress()` usage proves this is an **oversight**, not a design choice:
...
- Prior audit (Halborn Jul 2023, 5.3.5) flagged the same missing-`_checkAddress` pattern as valid — Kiln fixed those, proving the codebase _should_ have these checks
